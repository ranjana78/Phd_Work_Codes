...
def MMD(Sam):
  ...
  for i,j in zip(Set_A,range(len(Set_A))):
    ...
    S_Set_A=torch.stack(Set_A)
    Int_proto_B=torch.mean(S_Set_B,dim=0)
    Int_proto_A=torch.mean(S_Set_A,dim=0)
    Int_dis=euclidean_distance(Int_proto_A,Int_proto_B)
    Set_B=Set_A.copy()
    W_Vec.append(Element)
    W_Alpha.append(Int_dis)
  #---------------------Normalize the weights----------------------
  N_W_Alpha=Normalize_and_IF(W_Alpha)
  #----------------------------------------------------------------
  W_Vec=torch.stack(W_Vec)
  N_W_Alpha=torch.stack(N_W_Alpha)
  Weight_Vec=[]
  #-----------------Multiply IF factors with samples----------------
  for i,j in zip(W_Vec,N_W_Alpha):
    Weight_Vec.append(torch.mul(i,j))
  Weight_Vec=torch.stack(Weight_Vec)
  Mean_Vec=torch.mean(Weight_Vec, dim=0)
  Mean_IF=torch.mean(N_W_Alpha)
  #---------------create prototypes--------------------------------  
  Weight_Proto=torch.div(Mean_Vec,Mean_IF)
  return Weight_Proto
#normalization function and influence factor creator
def Normalize_and_IF(List):
  #List=List.cpu().data.numpy()
  min_val = min(List) 
  max_val = max(List) 
  normalized_values = [(x - min_val) / (max_val - min_val) for x in List] 
  IF_values=[]
  for i in normalized_values:
    IF_values.append(1-i)
  return IF_values  
...
def main():
  ...
  PATH_Model='/home/ranjana/New_Models/Journal/IPNET/'+'IPNET_CONV4_'+Data+'_n_way_'+str(n_way)+'_n_support1_'+str(n_support1)+'.pth'
